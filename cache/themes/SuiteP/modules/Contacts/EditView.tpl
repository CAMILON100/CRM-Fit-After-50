...
<div class="col-xs-12 col-sm-2 label" data-label="LBL_TYPE_CONTACT">
...
<div class="col-xs-12 col-sm-8 edit-view-field " type="enum" field="type_contact_c" colspan='3' >
{counter name="panelFieldCount" print=false}

{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.type_contact_c.name}" 
id="{$fields.type_contact_c.name}" 
title=''       
>
{if isset($fields.type_contact_c.value) && $fields.type_contact_c.value != ''}
{html_options options=$fields.type_contact_c.options selected=$fields.type_contact_c.value}
{else}
{html_options options=$fields.type_contact_c.options selected=$fields.type_contact_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.type_contact_c.options }
{capture name="field_val"}{$fields.type_contact_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.type_contact_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.type_contact_c.name}" 
id="{$fields.type_contact_c.name}" 
title=''          
>
{if isset($fields.type_contact_c.value) && $fields.type_contact_c.value != ''}
...
<input
id="{$fields.type_contact_c.name}-input"
name="{$fields.type_contact_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.type_contact_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.type_contact_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.type_contact_c.name}-input', '{$fields.type_contact_c.name}');sync_{$fields.type_contact_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.type_contact_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.type_contact_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.type_contact_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.type_contact_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.type_contact_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.type_contact_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.type_contact_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
addToValidate('EditView', 'type_contact_c', 'enum', true,'{/literal}{sugar_translate label='LBL_TYPE_CONTACT' module='Contacts' for_js=true}{literal}' );
