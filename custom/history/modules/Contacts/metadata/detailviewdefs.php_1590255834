    ...
    array (
      ...
      array (
        ...
        array (
          ...
          array (
            'customCode' => '<a type="button" class="button"   onclick="$(document).openComposeViewModal(this);" data-module="Contacts"  data-record-id="{$fields.id.value}" data-module-name="{$fields.full_name.value}" data-email-address="" style="padding-top:10px">{$MOD.LBL_SEND_EMAIL}</a>',
          ),
          'SEND_WHATSAPP' => 
          array (
            'customCode' => '<a type="button" class="button" onClick="window.open(`https://api.whatsapp.com/send?phone={$fields.phone_mobile.value}`, `width=600,height=600`);" style="padding-top:10px">{$MOD.LBL_SEND_WHATSAPP}</a>',
          ),
          0 => 'EDIT',
          1 => 'DUPLICATE',
          2 => 'DELETE',
