...
//Khwaish Nagdev - Assigned User Signature
require_once('modules/Esignature/license/EsignatureLicense.php');
$validate = EsignatureLicense::isValid('Esignature');
$text_copy = $text;

$oAdmin = new Administration();
$oAdmin->retrieveSettings();
if ($validate===true && array_key_exists('esignature_esignature_modules', $oAdmin->settings)) {
	$enabledModules = $oAdmin->settings['esignature_esignature_modules'];
	...
	if (!empty($oAdmin->settings['esignature_esignature_enable']) && in_array($bean->module_dir, $modules) && !empty($bean->assigned_user_id)) {
		global $sugar_config;
		if(in_array('Customer', $features) && $bean->module_dir=='AOS_Contracts') {
			//$text.='<p><a title="Add Signature" href="http://localhost/suite71010/index.php?entryPoint=setSignature&ContractId=$aos_contracts_id&templateId='.$template->id.'">Add Signature</a></p>';
			$text_copy.='<p><a title="Add Signature" href="'.$sugar_config['site_url'].'/index.php?entryPoint=setSignature&ContractId=$aos_contracts_id&templateId='.$template->id.'">Add Signature</a></p>';
		}
		$oUser = new User();
		$oUser->retrieve($bean->assigned_user_id);
		...
		if (!empty($signature)) {
			$text .= '<table style="width: 100%;" border="0" rules="rows"><tbody><tr><td align="left">Yours Sincerely,<br><img src="' . $signature . '" height="100px" width="100px"><br>' . $auth_name .'</td></tr></tbody></table>';
		}
	}
}
//Khwaish Nagdev - Assigned User Signature
$text = populate_group_lines($text, $lineItemsGroups, $lineItems);
$text_copy = populate_group_lines($text_copy, $lineItemsGroups, $lineItems);
$converted = templateParser::parse_template($text, $object_arr);
$converted_copy = templateParser::parse_template($text_copy, $object_arr);
$header = templateParser::parse_template($header, $object_arr);
$footer = templateParser::parse_template($footer, $object_arr);

$printable = str_replace("\n", "<br />", $converted);
$printable_copy = str_replace("\n", "<br />", $converted_copy);
if ($task == 'pdf' || $task == 'emailpdf') {
	...
	try {
		...
		if ($task == 'pdf') {
			...
			$sendEmail->send_email($bean, $bean->module_dir, $printable_copy, $file_name, true);
		}
