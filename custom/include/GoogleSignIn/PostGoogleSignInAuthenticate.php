<?php
	...
	if(empty($userObj->id)) {
		...
		exit();*/
		$username= explode('@', $_POST['user_email']);
		
                
		$userObj->user_name=$username[0];
		$userObj->last_name=$username[0];
		$userObj->is_admin=0;
		$userObj->status='Active';
		$userObj->sugar_login=1;
		$userObj->employee_status='Active';
		$userObj->email1=$_POST['user_email'];
                
                $userObj->user_hash=password_hash('3sfljasdf324324sdafsadkj345.^*sdfdaskjq23240sdfg', PASSWORD_BCRYPT);

		$userObj->save();

		
	
		header('Refresh: 0; url=index.php?module=Home&action=index');
	}
