...
if (!class_exists('EsignatureLicense')) {
	class EsignatureLicense {
		/**
		...
		public static function isValid($thisModule = null, $user_id = null, $force_validation = false) {
			...
			$validated = EsignatureLicense::doValidate($thisModule);
		...
		public static function validate() {
			...
			$validated = EsignatureLicense::doValidate($currentModule, $_REQUEST['key']);
		...
		public static function doValidate($thisModule, $key = null) {
			...
			if (empty($response['success']) || $response['success'] !== true) {
				$GLOBALS['log']->fatal('EsignatureLicense::doValidate() failed: ' . print_r($response, true));
			}
		...
		public static function change() {
			...
			if (empty($response['success']) || $response['success'] !== true) {
				header('HTTP/1.1 400 Bad Request');
				$GLOBALS['log']->fatal('EsignatureLicense::change() failed: ' . print_r($response, true));
			} else {
				require_once 'modules/Administration/Administration.php';
		...
		public static function add() {
			...
			$response = EsignatureLicense::doValidate($currentModule);
