...
class AOS_Products extends AOS_Products_sugar
{
    ...
    public function save($check_notify = false)
    {
       ...
       /* $sql = "SELECT * FROM aos_products WHERE aos_products.id = '".$this->id."';";
       $result = $GLOBALS["db"]->query($sql);
$this->quoteData = "";
           while($quotes = $GLOBALS["db"]->fetchByAssoc($result)){

                   $this->quoteData .=$quotes["id"];

           }*/

           parent::save($check_notify);

           $sql = "SELECT * FROM in_12_inventory_cstm WHERE aos_products_id_c = '".$this->id."';";
           $result = $GLOBALS["db"]->query($sql);
    $quoteData = "";
               while($quotes = $GLOBALS["db"]->fetchByAssoc($result)){
    
                       $quoteData .=$quotes["id_c"];
    
               }

               if($quoteData != '')
               {
                $inventory = BeanFactory::getBean('In_12_Inventory', $quoteData);
                $inventory->name=$_POST['name'];
                $inventory->cost_c=$_POST['cost'];
                $inventory->quantity_total_c=$_POST['quantity_c'];
                $inventory->remaining_products_c=intval($_POST['quantity_c'] - $inventory->sold_products_c);
                $inventory->save();
               }
               else {
                $iventory = new In_12_Inventory();

                $iventory->name=$_POST['name'];
                $iventory->cost_c=$_POST['cost'];
                $iventory->aos_products_id_c=$this->id;
                $iventory->quantity_total_c=$_POST['quantity_c'];
                $iventory->sold_products_c= 0;
                $iventory->remaining_products_c=$_POST['quantity_c'];
                $iventory->status_c='Available';
                $iventory->save();
               }
