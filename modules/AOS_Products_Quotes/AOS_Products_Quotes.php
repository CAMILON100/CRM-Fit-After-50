            ...
            if (isset($post_data[$key . 'deleted'][$i]) && $post_data[$key . 'deleted'][$i] == 1) {
                ...
                if (trim($product_quote->product_id) != '' && trim($product_quote->name) != '' && trim($product_quote->product_unit_price) != '') {
                    ...
                    $product_quote->save();
                      ...
                      $sql = "SELECT * FROM in_12_inventory_cstm WHERE aos_products_id_c = '".$product_quote->product_id."';";
...
$quoteData = "";
           while($quotes = $GLOBALS["db"]->fetchByAssoc($result)){
                   ...
                   $quoteData .=$quotes["id_c"];
                    ...
                    $Inventory->sold_products_c=$product_quote->product_qty;
                    $Inventory->remaining_products_c=intval($Inventory->quantity_total_c - $product_quote->product_qty);
                    $Inventory->save();
